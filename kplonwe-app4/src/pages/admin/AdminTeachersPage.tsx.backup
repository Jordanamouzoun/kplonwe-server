...
import { 
  ...
  ChevronDown, AlertCircle, User, FileText
} from 'lucide-react';
...
export function AdminTeachersPage() {
  ...
  const handleValidate = async (teacherId: string, status: 'VERIFIED' | 'REJECTED') => {
    ...
    try {
      await api.post(`/admin/teachers/${teacherId}/validate`, { 
        status,
  ...
  const statusConfig = {
    ...
    VERIFIED: { color: 'green', label: 'Vérifié', icon: CheckCircle },
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 sm:p-12 text-center">
            ...
            <p className="text-gray-600 text-sm sm:text-base">Aucun enseignant trouvé</p>
          </div>
        ) : (
          <>
            {/* Mobile: Cards */}
